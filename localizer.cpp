	...
	float p_miss) 
{
	...
	int hit;
	float p;
	vector<float> row (beliefs[0].size());
...
/*
int main(void) {
	vector < vector < char > > map = read_map("maps/m2.txt");
	vector < vector < float > > new_map;
	vector < vector < float > > updated_map;
	show_grid(map);
	new_map = initialize_beliefs(map);
	show_grid(new_map);
	float p_hit = 3.0;
	float p_miss = 1.0;
	updated_map = sense('r', map, new_map, p_hit, p_miss);
	show_grid(updated_map);
}
*/
